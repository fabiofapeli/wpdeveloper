	...
	class Unit extends CoursePress_Object {
		...
		function init_unit( &$unit ) {
			if ( ! empty( $unit ) ) {
				...
				if ( $unit->post_title == '' ) {
					$unit->post_title = __( 'Untitled', 'coursepress' );
				}
		...
		function get_unit_page_time_estimation( $unit_id, $page_num ) {
			...
			foreach ( $modules as $mod ) {
				...
				if ( $minutes >= 1 || $seconds >= 1 ) {
					...
					return apply_filters( 'coursepress_unit_time_estimation_na_format', __( 'N/A', 'coursepress' ) );
				}
			}
		}
		...
		function create_auto_draft( $course_id ) {
			...
			$post = array(
				...
				'post_title'   => __( 'Untitled', 'coursepress' ),
				'post_type'    => 'unit',
		...
		function update_unit() {
			...
			if ( isset( $_POST['unit_id'] ) && $_POST['unit_id'] != 0 ) {
				...
				if ( $_POST['unit_name'] !== '' && $_POST['unit_name'] !== __( 'Untitled', 'coursepress' ) /* && $_POST['unit_description'] !== '' */ ) {
