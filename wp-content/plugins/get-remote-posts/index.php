...
class KDM_Posts
{
	...
	function deactivation(){
		...
		delete_transient('grp'); // Reseta dados de transiente
	}
	...
	function options_form(){
			...
			if(isset($_GET['settings-updated']))
				delete_transient('grp'); // Reseta dados de transiente para que na próxima requisição atualize dados
	...
	function get_posts(){
		$posts = get_transient('grp'); //verifica tempo de transient
		if(!$posts){ //caso tempo de transiente tenha sido ultrapassado verifica postagens
			...
			$posts = array();
				...
				if(is_object($data) && isset($data->status)){
						if($data->status=='success'){
							...
							array_push($posts, 'Atualizado em ' . $tempo);
				...
				set_transient('grp', $posts, 60*60); //Define tempo de transiente em 1 hora para próxima requisição
			}
